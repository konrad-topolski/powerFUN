...
void FilterInterface(fftw_complex *field, std::string FilterConvolveName, std::string FilterDeconvolveName, double RadiusConvolve, double RadiusDeconvolve){
	...
	if(FilterDeconvolveName.compare("TopHat")==0){
		...
		TopHat(field,"Deconvolve", RadiusDeconvolve);	
	        ...
	        Gaussian(field,"Deconvolve", RadiusDeconvolve);   
                ...
                std::cout << "Applying NearestGridPoint deconvolution of radius " << RadiusDeconvolve << " Mpc/h" << std::endl;
	        FilterFamilyRadial(field,1,"Deconvolve", RadiusDeconvolve);   
	}
	else if(FilterDeconvolveName.compare("CIC")==0){
                std::cout << "Applying CloudInCell deconvolution of radius " << RadiusDeconvolve << " Mpc/h" << std::endl;
	        FilterFamilyRadial(field,2,"Deconvolve", RadiusDeconvolve);   
	}
	else if(FilterDeconvolveName.compare("TSC")==0){
                std::cout << "Applying TriangularShapedCloud deconvolution of radius " << RadiusDeconvolve << " Mpc/h" << std::endl;
	...
	if(FilterConvolveName.compare("TopHat")==0){
	        ...
	        Gaussian(field,"Convolve", RadiusConvolve);   
                ...
                std::cout << "Applying NearestGridPoint convolution of radius " << RadiusConvolve << " Mpc/h" << std::endl;
		...
		FilterFamilyRadial(field,1,"Convolve", RadiusConvolve);   
                ...
                std::cout << "Applying CloudInCell convolution of radius " << RadiusConvolve << " Mpc/h" << std::endl;
	        #ifdef RADIAL
                ...
                std::cout << "Applying TriangularShapedCloud convolution of radius " << RadiusConvolve << " Mpc/h" << std::endl;
	        #ifdef RADIAL
...
void FilterFamilyRadial(fftw_complex* field, int P, std::string mode, double Radius){
   ...
   double fsize= Radius*double(NGRID)/double(L);
...
void FilterFamily(fftw_complex* field, int P, std::string mode, double Radius){
   ...
   int ni,nf,icx,icy,icz,isx,isy,isz;
   float irz,iry,irx, zk, rks, tmp, cfilter=0.0;
   double kNyq = M_PI*double(NGRID)/L;
   field[LOC(0,0,0)][0] *= 1.0; field[LOC(0,0,0)][1] *= 1.0;
