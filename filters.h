...
//#define RADIAL // use the radial (sphere) version, or otherwise, multiplicative (cube) version of the filters 

void FilterInterface(fftw_complex *field, 
			std::string FilterConvolveName, std::string FilterDeconvolveName,
			 double RadiusConvolve, double RadiusDeconvolve);
...
// CIC, p=3 is W_R(r) = { 1/3 - (r/R)^2 if r<=1/3 * R,  0.5*(1-(r/R))^2 if 1/3 R <= r <= R and 0 otherwise
